import Image from "next/image";
import Link from "next/link";
import axios from "axios";
import { AiOutlineLike } from "react-icons/ai";

const FetchPopular = async (movie) => {
  const BASE_URL = `https://api.themoviedb.org/3/search/movie?query=${movie}&api_key=${process.env.API_KEY}`;
  const req = await axios.get(BASE_URL);
  const data = req.data;
  return data.results;
};

const SearchPage = async ({ params }) => {
  const id = params.id;
  const data = await FetchPopular(id);
  console.log(data);
  console.log(id);
  return (
    <div className="sm:grid flex max-w-[1100px] mx-auto mt-5 mb-10 xl:grid-cols-5 lg:grid-cols-3 md:grid-cols-2  flex-col sm:items-center sm:justify-center gap-5 grid-rows-4 overflow-hidden">
      {data.map((item) => {
        return (
          <Link
            href={`/movie/${item.id}`}
            key={item.id}
            className=" pb-5 hover:shadow-slate-400 transition cursor-pointer hover:shadow-md border-gray-500 rounded-lg border-2 flex flex-col justify-center"
          >
            <Image
              width={1920}
              height={1080}
              className="object-cover mx-auto min-w-[92%] max-w-[92%] min-h-[50%] max-h-[50%] overflow-hidden mt-2 rounded-lg"
              src={`https://image.tmdb.org/t/p/original${item.backdrop_path}`}
              placeholder="blur"
              blurDataURL="/spinner.svg"
              alt={item.title}
            />
            <p className="line-clamp-2  mx-3 mt-2  font-medium">
              {item.overview}
            </p>
            <p className="line-clamp-1 mx-3 mt-1 text-lg font-bold">{item.title}</p>
            <div className="flex mt-1 mx-3 items-center">
              <p className="mr-3">{item.release_date}</p>
              <AiOutlineLike className="mr-1" />
              <p>{item.vote_count}</p>
            </div>
          </Link>
        );
      })}
    </div>
  );
};

export default SearchPage;
